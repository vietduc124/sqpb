...
.bubble {{
    max-width:68%; padding:10px 16px; font-size:14px;
...
/* Streaming bubble (st.chat_message dùng khi đang stream) */
...
# ── Lịch sử chat — user phải, bot trái ────────────────────────────────────────
for msg in st.session_state.messages:
    role = msg["role"]
    content_html = md(msg["content"])
    side = "user" if role == "user" else "bot"
    st.markdown(
        f'<div class="chat-row {side}"><div class="bubble {side}">{content_html}</div></div>',
        unsafe_allow_html=True,
    )
...
if prompt := st.chat_input("Hãy nhập câu hỏi của bạn tại đây..."):
    ...
    # Stream vào bubble bot bên trái
